...
If needed, the script can load data from data files other than the default
with `--neofile` or `--cadfile`.
"""
import argparse
import cmd
import datetime
...
def date_fromisoformat(date_string):
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(f"'{date_string}' is not a valid "
                                         "date. Use YYYY-MM-DD.")
...
def make_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Explore past and future close approaches "
    ...
    inspect_id.add_argument('-p', '--pdes',
                            help="The primary designation of the NEO to "
                                 "inspect (e.g. '433').")
    ...
    query = subparsers.add_parser('query',
                                  description="Query for close "
                                              "approaches that match "
                                              "a collection of filters.")
    filters = query.add_argument_group('Filters',
                                       description="Filter close approaches by"
                                                   " their attributes or the "
                                                   "attributes of their NEOs.")
    ...
    filters.add_argument('-s', '--start-date', type=date_fromisoformat,
                         help="Only return close approaches on or after "
                              "the given date, in "
                              "YYYY-MM-DD format (e.g. 2020-12-31).")
    filters.add_argument('-e', '--end-date', type=date_fromisoformat,
                         help="Only return close approaches on or before"
                              "the given date, in YYYY-MM-DD format "
                              "(e.g. 2020-12-31).")
    filters.add_argument('--min-distance', dest='distance_min', type=float,
                         help="In astronomical units. Only return "
                              "close approaches that pass as "
                              "far or farther away from Earth as "
                              "the given distance.")
    filters.add_argument('--max-distance', dest='distance_max', type=float,
                         help="In astronomical units. Only return close "
                              "approaches that pass as near or nearer "
                              "to Earth as the given distance.")
    filters.add_argument('--min-velocity', dest='velocity_min', type=float,
                         help="In kilometers per second. Only return "
                              "close approaches whose relative "
                              "velocity to Earth at approach is "
                              "as fast or faster than the given velocity.")
    filters.add_argument('--max-velocity', dest='velocity_max', type=float,
                         help="In kilometers per second. Only return close "
                              "approaches whose relative velocity to Earth "
                              "at approach is as slow or slower than the "
                              "given velocity.")
    filters.add_argument('--min-diameter', dest='diameter_min', type=float,
                         help="In kilometers. Only return close approaches "
                              "of NEOs with diameters as large or larger "
                              "than the given size.")
    filters.add_argument('--max-diameter', dest='diameter_max', type=float,
                         help="In kilometers. Only return close approaches "
                              "of NEOs with diameters as small or smaller "
                              "than the given size.")
    filters.add_argument('--hazardous', dest='hazardous', default=None,
                         action='store_true',
                         help="If specified, only return close approaches "
                              "of NEOs that are potentially hazardous.")
    filters.add_argument('--not-hazardous', dest='hazardous', default=None,
                         action='store_false',
                         help="If specified, only return close approaches of "
                              "NEOs that are not potentially hazardous.")
    ...
    query.add_argument('-o', '--outfile', type=pathlib.Path,
                       help="File in which to save structured results. "
                            "If omitted, results are printed to standard "
                            "output.")

    repl = subparsers.add_parser('interactive',
                                 description="Start an interactive command "
                                             "session to repeatedly run "
                                             "`interact` and `query` "
                                             "commands.")
    repl.add_argument('-a', '--aggressive', action='store_true',
                      help="If specified, kill the session whenever a project "
                      "file is modified.")
...
def inspect(database, pdes=None, name=None, verbose=False):
    ...
    This function fetches an NEO by designation or by name. If a matching
    NEO is found, information about the NEO is printed (additionally,
    information for all of the NEO's known close approaches is printed
    if `verbose=True`). Otherwise, a message is printed noting that there
    are no matching NEOs.

    At least one of `pdes` and `name` must be given. If both are given, prefer
    to look up the NEO by the primary designation.

    :param database: The `NEODatabase` containing data on NEOs and their
    close approaches.
    :param pdes: The primary designation of an NEO for which to search.
    :param name: The name of an NEO for which to search.
    :param verbose: Whether to additionally print all of a matching NEO's
    close approaches.
    ...
    # Display information about this NEO, and optionally its close
    # approaches if verbose.
    print(neo)
    if verbose:
...
def query(database, args):
    ...
    use the file's extension to infer whether the file should hold CSV or
    JSON data, and then write the results to the output file in that format.

    :param database: The `NEODatabase` containing data on NEOs and their
    close approaches.
    :param args: All arguments from the command line, as parsed by the
    top-level parser.
    """
    # Construct a collection of filters from arguments supplied at the
    # command line.
    ...
    if not args.outfile:
        ...
        if args.outfile.suffix == '.csv':
            ...
            print("Please use an output file that ends with `.csv`"
                  "or `.json`.", file=sys.stderr)


class NEOShell(cmd.Cmd):
    """Perform the `interactive` subcommand.

    This is a `cmd.Cmd` shell - a specialized tool for command-based REPL
    sessions.

    It wraps the `inspect` and `query` parsers to parse flags for
    those commands as if they were supplied at the command line.
    ...
    def __init__(self, database, inspect_parser, query_parser,
                 aggressive=False, **kwargs):
        """Create a new `NEOShell`.

        Creating this object doesn't start the session - for that, use
        `.cmdloop()`.
        ...
        :param aggressive: Whether to kill the session whenever a project
        file is changed.
        :param kwargs: A dictionary of excess keyword arguments passed to the
        superclass.
        """
    ...
    def parse_arg_with(cls, arg, parser):
        ...
        :return: A `Namespace` of the arguments (produced by `parse_args`) or
        None.
        """
    ...
    def do_query(self, arg):
        ...
        This command behaves the same as the `query` subcommand from the
        command line. For example, to query close approaches on January 1st,
        2020:
        ...
        The number of results shown can be limited to a maximum number with
        `--limit`:

            (neo) query --limit 2

        The results can be saved to a file (instead of displayed to stdout)
        with `--outfile`:
    ...
    def precmd(self, line):
        ...
        files = ', '.join(str(f.relative_to(PROJECT_ROOT)) for f in changed)
        if changed:
            ...
            if not self.aggressive:
                print("To include these changes, please exit and restart this"
                      "interactive session.", file=sys.stderr)
            else:
